...
- 增加Boss血条信息显示：血条内左侧显示当前血量、血条下方显示 Boss 名字（白字黑描边，直接画在血条上）
- 自动接任务的提示灯泡改为固定在屏幕左侧（官方怀旧服的处理方式），不再挂在角色头顶挡视线
- 增加buff图标倒计时（官方怀旧服的处理方式）：图标左下角显示数字，1分钟以上绿色显示向上取整的分钟数、1分钟以内黄色显示向上取整的秒数、10分钟以上不显示（阈值/字号/颜色见 config.ini 的 buffTimer*；逆向记录见 docs/客户端逆向-Buff图标倒计时.md）
